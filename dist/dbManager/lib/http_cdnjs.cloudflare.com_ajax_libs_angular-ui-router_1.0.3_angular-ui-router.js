...
(function (global, factory) {
    (typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? factory(exports, require('angular')) : typeof define === 'function' && define.amd ? define(['exports', 'angular'], factory) : factory(global['@uirouter/angularjs'] = global['@uirouter/angularjs'] || {}, global.angular);
})(undefined, function (exports, ng_from_import) {
    'use strict';
    ...
    var is = function is(ctor) {
        return function (obj) {
            return obj != null && obj.constructor === ctor || obj instanceof ctor;
        };
    };
    /** Given a value, returns a Predicate function that returns true if another value is === equal to the original value */
    var eq = function eq(val) {
    ...
    var notImplemented = function notImplemented(fnname) {
        return function () {
     ...
     */
    var inherit = function inherit(parent, extra) {
    ...
    var deregAll = function deregAll(functions) {
    ...
    var mergeR = function mergeR(memo, item) {
     ...
     * ```
     */
    var allTrueR = function allTrueR(memo, elem) {
        return memo && elem;
    };
    /**
     ...
     * ```
     */
    var anyTrueR = function anyTrueR(memo, elem) {
        return memo || elem;
    };
    /**
     ...
     * ```
     */
    var unnestR = function unnestR(memo, elem) {
     ...
     * ```
     */
    var flattenR = function flattenR(memo, elem) {
    ...
    var uniqR = function uniqR(acc, token) {
     ...
     */
    var sortBy = function sortBy(propFn, checkFn) {
        if (checkFn === void 0) {
            checkFn = val(true);
        }
        return function (a, b) {
     ...
     */
    var composeSort = function composeSort() {
        var sortFns = [];
        for (var _i = 0; _i < arguments.length; _i++) {
    ...
    // issue #2676
    var silenceUncaughtInPromise = function silenceUncaughtInPromise(promise) {
        return promise.catch(function (e) {
            return 0;
        }) && promise;
    };
    var silentRejection = function silentRejection(error) {
    ...
    var Trace = function () {
        ...
        Trace.prototype._set = function (enabled, categories) {
            ...
            if (!categories.length) {
                categories = Object.keys(exports.Category).map(function (k) {
        ...
        Trace.prototype.traceTransitionStart = function (trans) {
            if (!this.enabled(exports.Category.TRANSITION)) return;
        ...
        Trace.prototype.traceResolvePath = function (path, when, trans) {
            if (!this.enabled(exports.Category.RESOLVE)) return;
        ...
        Trace.prototype.traceUIViewEvent = function (event, viewData, extra) {
            ...
            if (!this.enabled(exports.Category.UIVIEW)) return;
    ...
    var Param = function () {
        ...
        Param.prototype.value = function (value) {
            ...
            var replaceSpecialValues = function replaceSpecialValues(val$$1) {
    ...
    var Transition = function () {
        ...
        Transition.prototype.is = function (compare) {
            ...
            return !(compare.to && !matchState(this.$to(), compare.to) || compare.from && !matchState(this.$from(), compare.from));
        };
        ...
        Transition.prototype.error = function () {
            ...
            if (!Param.validates(state.parameters(), this.params())) return "Param values not valid for state '" + state.name + "'";
    ...
    var stringifyPattern = function stringifyPattern(value) {
        ...
        stringifyPatternFn = stringifyPatternFn || pattern([[not(isDefined), val("undefined")], [isNull, val("null")], [isPromise, val("[Promise]")], [isRejection, function (x) {
            return x._transitionRejection.toString();
        }], [is(Rejection), invoke("toString")], [is(Transition), invoke("toString")], [is(Resolvable), invoke("toString")], [isInjectable, functionToString], [val(true), identity]]);
    ...
    function stringify(o) {
        ...
        function format(val$$1) {
            if (isObject(val$$1)) {
                if (seen.indexOf(val$$1) !== -1) return '[circular ref]';
                seen.push(val$$1);
            }
    ...
    function initDefaultTypes() {
        var makeDefaultType = function makeDefaultType(def) {
            var valToString = function valToString(val$$1) {
        ...
        extend(ParamTypes.prototype, {
            ...
            date: makeDefaultType({
                encode: function encode(val$$1) {
                    return !this.is(val$$1) ? undefined : [val$$1.getFullYear(), ('0' + (val$$1.getMonth() + 1)).slice(-2), ('0' + val$$1.getDate()).slice(-2)].join("-");
                },
                decode: function decode(val$$1) {
    ...
    /** @hidden */
    var memoizeTo = function memoizeTo(obj, prop$$1, fn) {
    ...
    var UrlMatcher = function () {
        ...
        UrlMatcher.prototype.validates = function (params) {
            var validParamVal = function validParamVal(param, val$$1) {
        ...
        UrlMatcher.compare = function (a, b) {
            ...
            var cmp,
                i,
                pairs$$1 = arrayTuples(weights(a), weights(b));
    ...
    var UrlRuleFactory = function () {
        ...
        UrlRuleFactory.prototype.fromUrlMatcher = function (urlMatcher, handler) {
            ...
            if (is(UrlMatcher)(handler)) _handler = function _handler(match) {
    ...
    var ViewService = function () {
        ...
        ViewService.prototype.registerUIView = function (uiView) {
            ...
            if (uiViews.filter(fqnMatches).length) trace.traceViewServiceUIViewEvent("!!!! duplicate uiView named:", uiView);
            uiViews.push(uiView);
            this.sync();
            return function () {
    ...
    function addCoreResolvables(trans) {
        trans.addResolvable({ token: UIRouter, deps: [], resolveFn: function resolveFn() {
                return trans.router;
            }, data: trans.router }, "");
    ...
    var registerAddCoreResolvables = function registerAddCoreResolvables(transitionService) {
    ...
    var registerRedirectToHook = function registerRedirectToHook(transitionService) {
    ...
    var registerOnExitHook = function registerOnExitHook(transitionService) {
    ...
    var registerOnRetainHook = function registerOnRetainHook(transitionService) {
    ...
    var registerOnEnterHook = function registerOnEnterHook(transitionService) {
    ...
    var eagerResolvePath = function eagerResolvePath(trans) {
        return new ResolveContext(trans.treeChanges().to).resolvePath("EAGER", trans).then(noop$1);
    };
    var registerEagerResolvePath = function registerEagerResolvePath(transitionService) {
    ...
    var lazyResolveState = function lazyResolveState(trans, state) {
        return new ResolveContext(trans.treeChanges().to).subContext(state.$$state()).resolvePath("LAZY", trans).then(noop$1);
    };
    var registerLazyResolveState = function registerLazyResolveState(transitionService) {
    ...
    var registerLoadEnteringViews = function registerLoadEnteringViews(transitionService) {
    ...
    var registerActivateViews = function registerActivateViews(transitionService) {
    ...
    var registerUpdateGlobalState = function registerUpdateGlobalState(transitionService) {
    ...
    var registerUpdateUrl = function registerUpdateUrl(transitionService) {
    ...
    var registerLazyLoadHook = function registerLazyLoadHook(transitionService) {
    ...
    var registerIgnoredTransitionHook = function registerIgnoredTransitionHook(transitionService) {
    ...
    var registerInvalidTransitionHook = function registerInvalidTransitionHook(transitionService) {
    ...
    var $q = {
        ...
        when: function when(val$$1) {
            return new Promise(function (resolve, reject) {
                return resolve(val$$1);
            });
        },
        /** Normalizes a value as a promise rejection */
        reject: function reject(val$$1) {
            return new Promise(function (resolve, reject) {
                reject(val$$1);
            });
        },
     ...
     */
    /** */
    var beforeAfterSubstr$1 = function beforeAfterSubstr$1(char) {
        return function (str) {
            if (!str) return ["", ""];
            var idx = str.indexOf(char);
            if (idx === -1) return [str, ""];
    ...
    var trimHashVal = function trimHashVal(str) {
        return str ? str.replace(/^#/, "") : "";
    };
    var keyValsToObjectR = function keyValsToObjectR(accum, _a) {
        var key = _a[0],
            val$$1 = _a[1];
    ...
    var getParams = function getParams(queryString) {
    ...
    var buildUrl = function buildUrl(loc) {
        var path = loc.path();
    ...
    function servicesPlugin(router) {
        ...
        services.$q = $q;
        return { name: "vanilla.services", $q: $q, $injector: $injector, dispose: function dispose() {
                return null;
            } };
    }
    ...
    var getStateProvider = function getStateProvider() {
        return extend(router.stateProvider, { $get: function $get() {
    ...
    ng.module('ui.router.state').directive('uiSref', uiSref).directive('uiSrefActive', uiSrefActive).directive('uiSrefActiveEq', uiSrefActive).directive('uiState', uiState);
     ...
     */
    var uiView;
    uiView = ['$view', '$animate', '$uiViewScroll', '$interpolate', '$q', function $ViewDirective($view, $animate, $uiViewScroll, $interpolate, $q$$1) {
...
//# sourceMappingURL=http_cdnjs.cloudflare.com_ajax_libs_angular-ui-router_1.0.3_angular-ui-router.js.map