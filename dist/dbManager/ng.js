...
var app = angular.module('admin', ["ui.router", "ng-file-model"]).run(function ($rootScope, $state) {
	/*$http({
 		method: 'POST',
...
app.config(function ($stateProvider, $urlRouterProvider) {
	...
	$stateProvider.state('CMS', {
		...
		controller: function controller($scope, $http, $state, $location, $rootScope, CRUD) {
			console.log('fuck');
			$scope.crud = CRUD;
		...
		controller: function controller($scope, CRUD, $http) {
			$scope.crud = CRUD;
		...
		controller: function controller($scope, $http, $stateParams) {
			...
			$scope.submitNewArticle = function (valid) {
				...
				if ($scope.action == 'edit') {
					...
					$http({
						...
						if (res.data == 'update success') toastr["success"]('המאמר נערך בהצלחה!');else if (res.data == 'query failed') toastr["error"]('יצירת המאמר נכשלה , כשל במאגר הנתונים');else {
					...
					$http({
						...
						console.log(res);
						console.log(res.data);
						if (res.data == 'upload success') toastr["success"]('המאמר נוצר בהצלחה!');else if (res.data == 'query failed') toastr["error"]('יצירת המאמר נכשלה , כשל במאגר הנתונים');else {
		...
		controller: function controller($scope, CRUD, $http, $stateParams) {
		...
		controller: function controller($scope, CRUD, $http, $stateParams) {
			...
			$scope.getCateIdLists = function () {
				$http.get('../class/DB.php?list=cates_id_lists').then(function (res) {
					$scope.selectedArts = res.data;
					console.log(res.data);
				});
			};
			$scope.getCateIdLists();
			$scope.quickArtSearch = function (e) {
				var val = e.target.value;
				console.log(val);
				$http.get("../class/DB.php?act=quick-art-search&qt=" + val).then(function (res) {
					console.log(res);
			...
			$scope.selectedArts = [];
			$scope.tab = 'search';
			$scope.addToList = function (item, category) {
				var map = new Map($scope.selectedArts[category]);
				map.set(item.id, item);
				$scope.selectedArts[category] = Array.from(map);
			};
			$scope.sendSelectedCates = function (cate) {
				//console.log($("#sortable"+cate));

				var idsInOrder = $('#sortable' + cate).sortable("toArray");
				$http.get("../class/DB.php?act=update-arts-id&order-ids=" + idsInOrder + "&category=" + cate).then(function (res) {
					console.log(res);
					console.log(res.data);
					//$scope.quickResults = res.data;
					var oldArr = new Map($scope.selectedArts[cate]),
					    newArr = [];
					for (var x = 0; x < oldArr.size; x++) {
						var oldItem = oldArr.get(idsInOrder[x]);
			...
			$scope.deleteCateArt = function (id, cate, cateID) {
				$http.delete("../class/DB.php?act=delete-cate-art&art-id=" + id + "&category=" + cateID).then(function (res) {
					console.log(res.data);
					var map = new Map($scope.selectedArts[cate]);
					map.delete(id);
		...
		controller: function controller($scope, CRUD, $stateParams) {
			...
			$scope.crud.getTableQuan('comm').then(function (res) {
				...
				if ($scope.query) $scope.crud.goSearch($scope, $scope.query);else $scope.crud.getCommAndApply($scope, $scope.page + 'Comm');
			});
			console.log($scope);
		},
		...
		controller: function controller($scope, CRUD, $stateParams) {
			...
			$scope.pageNum = $stateParams['pageNum'];
			$scope.query = $stateParams['query'];
			$scope.crud.getReporterList($scope.pageNum, $scope.query).then(function (data) {
		...
		controller: function controller($scope, CRUD) {
			$scope.crud = CRUD;
